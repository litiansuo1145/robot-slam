...
#include "../include/pavo2s_filter.h"
...
inline double getAngleWithViewpoint(double r1, double r2, double included_angle)
{
...
pavo_response_scan_t* MADataFilter::HandlTialFilterWithAngle(pavo_response_scan_t* data_buffer, int count)
{
    ...
    distance_std = distance_def*1.0f / 0.2f;

    std::set<int> indices_to_delete;
    double dandiao1 = 1.0f, dandiao2 = 1.0f;
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        for (int y = -window_; y < window_ + 1; y++)
        {
            ...
            angle1 = getAngleWithViewpoint(
                        float(copy_ptr_org[i].distance),
                        float(copy_ptr_org[j].distance),
                        y * Degree2Radians(double((copy_ptr_org[i + 1].angle - copy_ptr_org[i].angle) / 100.0)));
    ...
    for (std::set<int>::iterator it = indices_to_delete.begin(); it != indices_to_delete.end(); ++it)
    {
        ...
        if(copy_ptr[*it].distance == 0)
            copy_ptr[*it].intensity = 0;
    }
    memcpy(data_buffer, copy_ptr, sizeof(pavo_response_scan_t)*count);
    delete[] copy_ptr;
    delete[] copy_ptr_org;
